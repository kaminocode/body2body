...
model.load_state_dict(torch.load(weight_name,map_location=lambda storage, loc: storage))
#model.cuda()
model.float()
model.eval()
...
test_image ='../datasets/superman/train/181.png'
folder='../datasets/superman/train'
    ...
    if(nA != 0 and nB != 0):
        ...
        for i in range(nA):
            for j in range(nB):
                ...
                if norm==0:
                    norm=0.000000000000000000000001
...
cv2.imwrite('result1.png',canvas)   

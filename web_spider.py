...
if __name__ == "__main__":
    ...
    while True:
        ...
        if i%4==0:
            ...
            #data1['B_paths']='145.png'
            #data1['B']=temp2
        
            tmodel.set_input(data1)
            tmodel.test()
            visuals = tmodel.get_current_visuals()
            # Display the resulting frame
            img_path = tmodel.get_image_paths()
            visualizer.save_images(webpage, visuals, img_path)
            image=cv2.imread('/home/sachin/rahul/pose_estimation/results/dance_mimic/test_latest/images/0_fake_B.png')
            #cv2.imwrite("1.png", visuals['fake_B'])
            cv2.imshow('Video',image)
            #cv2.imshow('Video',canvas)
            #print(np.amax(visuals['fake_B']))
            #print(data1)
            #print(visuals['fake_B'].shape)
            #for label, image_numpy in visuals.items():
                #cv2.imshow('Video', image_numpy)
